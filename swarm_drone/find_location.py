...
def find_location(real_width,real_high,img):
    ...
    # show the img whith the point
    
    if squre_point is not None:
        for point in squre_point:
...
specific_filename = "output"

full_file_path = os.path.join(folder_path, specific_filename)
print("Script Folder:", full_file_path)

#folder_path = "./swarm_sdk/new_test"
files = os.listdir(full_file_path)
if files:
    for file_name in files:
        
        img = cv2.imread(f"{full_file_path}\{file_name}")
        
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        #plt.imshow(img)
        #plt.show()
        loc=find_location(29.7,21,img)
        if loc is not None:
